...
---`);
  ...
  if (req.types.includes('reviews')) {
    const reviewData = req.reviews?.trim()
      ? `\n아래 앱 리뷰 데이터를 분석하세요 (출처 번호가 있는 경우 [출처 N] 형식으로 인용하세요):\n\`\`\`\n${req.reviews}\n\`\`\``
...
---`);
  ...
  if (req.types.includes('news')) {
    const newsData = req.news?.trim()
      ? `\n아래 뉴스/자료를 분석하세요 (출처 번호가 있는 경우 [출처 N] 형식으로 인용하세요):\n\`\`\`\n${req.news}\n\`\`\``
...
---`);
  ...
  if (req.types.includes('research')) {
    const researchData = req.researchData?.trim()
      ? `\n아래 학술/연구 자료를 분석하세요 (출처 번호가 있는 경우 [출처 N] 형식으로 인용하세요):\n\`\`\`\n${req.researchData}\n\`\`\``
...
${req.types.includes('self') ? `**[시스템 지시]** 위 분석이 완전히 끝난 직후, 아래 형식의 JSON 점수 블록을 **반드시** 정확히 이 형식 그대로 출력하세요. 자체 서비스 진단 결과를 바탕으로 각 수치를 채워 넣으세요. 형식을 절대 바꾸지 마세요.
...
<!-- INSIGHT_SCORES
{
  ...
  "strengths": [
    {"name": "<강점명 6자 이내>", "score": <60-100 정수>, "description": "<30자 이내 한 줄>", "reasoning": "<이 강점이 분석에서 확인된 구체적 근거 1문장>"}
  ],
  "weaknesses": [
