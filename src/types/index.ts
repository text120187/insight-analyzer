...
export const ANALYSIS_TYPE_LABELS: Record<AnalysisType, { label: string; icon: string; description: string }> = {
  ...
  reviews:     { label: '앱 리뷰 분析',   icon: '⭐', description: '리뷰 데이터에서 Pain Point 도출' },
  news:        { label: '뉴스 분析',      icon: '📰', description: '뉴스·기사에서 산업 시사점 도출' },
  self:        { label: '자체 서비스 진단', icon: '🔍', description: '우리 서비스 강약점·개선 방향 분析' },
  research:    { label: '학술/연구 자료',  icon: '📚', description: '논문·리포트에서 근거 기반 인사이트 도출' },
  ux:         { label: 'UI/UX 화면 분析',  icon: '🖥️', description: '화면 캡처·URL로 사용성 검증' },
};
